...
{
    public class PcSupplementDetailService : IPcSupplementDetailService
    {

        public IRepositoryGUID<PcSupplementDetail> EntityRepository { get; set; }

        public IRepositoryGUID<GoodsArchives> GoodsRepository { get; set; }

        [Transaction]
        public string Create(PcSupplementDetail entity)
        {
            return EntityRepository.Save(entity);
        }

        [Transaction]
        public PcSupplementDetail GetById(string id)
        {
            return EntityRepository.Get(id);
        }

        [Transaction]
        public IList<PcSupplementDetail> GetAll()
        {
        ...
        [Transaction]
        public void Update(PcSupplementDetail entity)
        {
            EntityRepository.Update(entity);
        }

        [Transaction]
        public void Delete(PcSupplementDetail entity)
        {
            EntityRepository.Delete(entity);
        }

        [Transaction]
        public void Delete(IEnumerable<PcSupplementDetail> entitys)
        {
        ...
        [Transaction]
        public IList<PcSupplementDetail> GetDetailsByManageId(string manageId)
        {
            var q = from l in EntityRepository.LinqQuery where l.ManageId == manageId select l;
            if (q.Count() == 0)
            {
                return new List<PcSupplementDetail>();
            }
        ...
        private void FillGoodsName(IList<PcSupplementDetail> details)
        {
        ...
        [Transaction]
        public SearchResult<PcSupplementDetail> Search(SearchDtoBase<PcSupplementDetail> c)
        {
