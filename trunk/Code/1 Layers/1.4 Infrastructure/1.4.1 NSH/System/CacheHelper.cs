    ...
    public static class CacheHelper
    {
        ...
        public static object GetCache(string Key, object Obj, string FilePath, FileType Ft)
        {
            if (GetCache(Key) == null)
            {
                ...
                {
                    case FileType.Xml:
                        SetCache(Key, XmlHelper.Read(FilePath), FilePath);
                        break;

                    case FileType.Html:
                        SetCache(Key, FileHelper.Read(FilePath), FilePath);
                        break;

                    case FileType.Model:
                        SetCache(Key, XmlHelper.Read((Type)Obj, FilePath), FilePath);
                        break;
                }
            }
            return GetCache(Key);
        ...
        public static List<TResult> AsCache<TResult>(this IQueryable<TResult> dt) where TResult : class, new()
        {
            ...
            if (CacheHelper.GetCache(catch_Name) == null)
            {
                CacheHelper.SetCache(catch_Name, dt.ToList(), 30);
            }
            return (List<TResult>)CacheHelper.GetCache(catch_Name);
        }
        public static void ClearListCache<TResult>(this TResult dt) where TResult : class, new()
        {
            string catch_Name = "LIST_" + typeof(TResult).ToS();
            CacheHelper.Clear(catch_Name);
        }
        ...
        public static List<TResult> AsCache<TResult>(this IQueryable<TResult> dt, string Key) where TResult : class, new()
        {
            ...
            if (CacheHelper.GetCache(catch_Name) == null)
            {
                CacheHelper.SetCache(catch_Name, dt.ToList());
            }
            return (List<TResult>)CacheHelper.GetCache(catch_Name);
        }
        #endregion
