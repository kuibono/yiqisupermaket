...
{
    public class MsCancelCardManageService:IMsCancelCardManageService
    {

        public IRepositoryGUID<MsCancelCardManage> EntityRepository { get; set; }

        [Transaction]
        public string Create(MsCancelCardManage entity)
        {
        ...
        [Transaction]
        public MsCancelCardManage GetById(string id)
        {
            return EntityRepository.Get(id);
        }

        [Transaction]
        public IList<MsCancelCardManage> GetAll()
        {
        ...
        [Transaction]
        public void Update(MsCancelCardManage entity)
        {
            EntityRepository.Update(entity);
        }

        [Transaction]
        public void Delete(MsCancelCardManage entity)
        {
            EntityRepository.Delete(entity);
        }

        [Transaction]
        public void Delete(IEnumerable<MsCancelCardManage> entitys)
        {
        ...
        [Transaction]
        public SearchResult<MsCancelCardManage> Search(SearchDtoBase<MsCancelCardManage> c)
        {
            ...
            if (c.entity != null)
            {
				if (string.IsNullOrEmpty(c.entity.Id) == false)
                {
					...
					if (string.IsNullOrEmpty(c.entity.CancelReason) == false)
					{
						q = q.Where(p => p.CancelReason.Contains(c.entity.CancelReason));
					}
		...
		[Transaction]
        public IList<MsCancelCardManage> Search(string key, int pageSize = 20, int pageIndex = 1)
        {
            var q = EntityRepository.LinqQuery;
        ...
        public string GenerateCancelNumber()
        {
            ...
            {
                maxNumber = maxNumber.Replace("55", "");
                long maxNumberInt = Convert.ToInt64(maxNumber);

                return "55" + (maxNumberInt + 1).ToString("0000000000");
            }
        }
