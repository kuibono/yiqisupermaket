...
{
    public class MsReclaimCardManageService:IMsReclaimCardManageService
    {

        public IRepositoryGUID<MsReclaimCardManage> EntityRepository { get; set; }

        [Transaction]
        public string Create(MsReclaimCardManage entity)
        {
        ...
        [Transaction]
        public MsReclaimCardManage GetById(string id)
        {
            return EntityRepository.Get(id);
        }

        [Transaction]
        public IList<MsReclaimCardManage> GetAll()
        {
        ...
        [Transaction]
        public void Update(MsReclaimCardManage entity)
        {
            EntityRepository.Update(entity);
        }

        [Transaction]
        public void Delete(MsReclaimCardManage entity)
        {
            EntityRepository.Delete(entity);
        }

        [Transaction]
        public void Delete(IEnumerable<MsReclaimCardManage> entitys)
        {
        ...
        [Transaction]
        public SearchResult<MsReclaimCardManage> Search(SearchDtoBase<MsReclaimCardManage> c)
        {
            ...
            if (c.entity != null)
            {
				if (string.IsNullOrEmpty(c.entity.Id) == false)
                {
					...
					if (string.IsNullOrEmpty(c.entity.ReclaimReason) == false)
					{
						q = q.Where(p => p.ReclaimReason.Contains(c.entity.ReclaimReason));
					}
			...
			if (string.IsNullOrEmpty(c.key)==false)
            {
				q = from l in q
					...
					|| l.ReclaimReason.Contains(c.key)
		...
		[Transaction]
        public IList<MsReclaimCardManage> Search(string key, int pageSize = 20, int pageIndex = 1)
        {
            var q = EntityRepository.LinqQuery;
        ...
        public string GenerateReclaimNumber()
        {
            ...
            {
                maxNumber = maxNumber.Replace("58", "");
                long maxNumberInt = Convert.ToInt64(maxNumber);

                return "58" + (maxNumberInt + 1).ToString("0000000000");
            }
        }
