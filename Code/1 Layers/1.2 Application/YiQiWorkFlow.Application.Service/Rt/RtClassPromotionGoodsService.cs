...
{
    public class RtClassPromotionGoodsService:IRtClassPromotionGoodsService
    {

        public IRepositoryGUID<RtClassPromotionGoods> EntityRepository { get; set; }

        [Transaction]
        public string Create(RtClassPromotionGoods entity)
        {
        ...
        [Transaction]
        public RtClassPromotionGoods GetById(string id)
        {
            return EntityRepository.Get(id);
        }

        [Transaction]
        public IList<RtClassPromotionGoods> GetAll()
        {
        ...
        [Transaction]
        public void Update(RtClassPromotionGoods entity)
        {
            EntityRepository.Update(entity);
        }

        [Transaction]
        public void Delete(RtClassPromotionGoods entity)
        {
            EntityRepository.Delete(entity);
        }

        [Transaction]
        public void Delete(IEnumerable<RtClassPromotionGoods> entitys)
        {
        ...
        [Transaction]
        public SearchResult<RtClassPromotionGoods> Search(SearchDtoBase<RtClassPromotionGoods> c)
        {
            ...
            if (c.entity != null)
            {
				...
				if (string.IsNullOrEmpty(c.entity.Id) == false)
                {
					...
					if (string.IsNullOrEmpty(c.entity.GoodsClassName) == false)
					{
					 ...
					 if (c.entity.SaleDiscountRate > 0)
					{
						q = q.Where(p => p.SaleDiscountRate == c.entity.SaleDiscountRate);
					}
		...
		[Transaction]
        public IList<RtClassPromotionGoods> Search(string key, int pageSize = 20, int pageIndex = 1)
        {
            var q = EntityRepository.LinqQuery;
